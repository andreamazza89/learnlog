...
foo = LogEntry <$> description <*> notes <*> length <*> tags <*>
...
handleEvent state@(AppState entries fstate) event =
  case toMyEvent fstate event of
    ...
    ListEvent (UpdateList vtyEvent) -> handleListEventVi handleListEvent vtyEvent entries >>= continue . (`AppState` fstate)
    FormEvent ExitForm -> continue (AppState (listRemove 0 entries) Viewing)
    FormEvent (SaveEntry f) -> continue $ saveEntry f entries
    FormEvent (UpdateForm f) -> handleFormEvent event f >>= continue . AppState entries . Creating
...
createNewEntry entries currentState =
  fromMaybe
    currentState
    ( do
        (_, newEntry) <- listSelectedElement entries
        return $ AppState (selectFirstItem . addAtTop newEntry $ entries) (Creating $ mkForm newEntry)
    )
  where
